...
object ZPoolTest {
  ...
  private case class DefaultPool[R, E, A](
    ...
    def get(implicit trace: Trace): ZIO[Scope, E, A] = {
      ...
      def acquire: UIO[Attempted[E, A]] = for {
        ms1 <- currentMs
        out <- isShuttingDown.get.flatMap { down =>
          if (down) ZIO.interrupt
            ...
            state.modify { case State(size, free) =>
              if (free > 0 || size >= range.end)
                (
                  for {
                    beforeTake <- currentMs
                    attempted  <- items.take
                    afterTake  <- currentMs
                    _          <- Console.printLine(s"take: ${afterTake - beforeTake}").orDie

                    out <- attempted.result match {
                      case Exit.Success(item) =>
                        invalidated.get.flatMap { set =>
                          val isContained = set.contains(item)
                          if (isContained) finalizeInvalid(attempted) *> acquire
                ...
                Console.printLine("no free item!").orDie *> allocate *> acquire -> State(size + 1, free + 1)
              } else
                ZIO.interrupt -> State(size, free)
            }.flatten
        }
        ms2 <- currentMs
        _   <- Console.printLine(s"!!!ZPoolTest.DefaultPool.acquire: ${ms2 - ms1}ms").orDie
      } yield out
      ...
      def release(attempted: Attempted[E, A]): UIO[Any] =
        attempted.result match {
          case Exit.Success(item) =>
            invalidated.get.flatMap { set =>
              val isContained = set.contains(item)
              if (isContained) finalizeInvalid(attempted)
              else
      ...
      for {
        ...
        ms2                 <- currentMs
        _                   <- Console.printLine(s"ZPoolTest.DefaultPool.acquire: ${ms2 - beginMs}ms").orDie
        (release, attempted) = releaseAndAttempted
        _     <- release.when(attempted.isFailure)
        item  <- attempted.toZIO
        endMs <- currentMs
        _     <- Console.printLine(s"ZPoolTest.DefaultPool.get: ${endMs - beginMs}ms").orDie
      } yield item
    }
